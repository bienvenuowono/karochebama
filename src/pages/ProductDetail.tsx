...
export default function ProductDetail() {
  ...
  const unitLabel = product.typeId === 1 ? 'Régime' : (product.typeId === 2 ? 'Litre' : 'Unité');
              ...
              <div className="flex flex-col gap-2 mb-8">
                ...
                {product.typeId === 1 && product.priceTonne && parseFloat(product.priceTonne) > 0 && (
                  <div className="text-base font-semibold text-emerald-700">
                    Soit <span className="underline">{Number(product.priceTonne).toLocaleString('fr-FR')} CFA</span> la Tonne
                  </div>
                )}
              </div>
