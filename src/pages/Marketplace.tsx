...
export default function Marketplace() {
  ...
  const filteredProducts = useMemo(() => {
    ...
    const normSelectedSite = normalize(selectedSite);
    ...
    return products.filter(p => {
      ...
      const matchesSearch = normalize(p.name).includes(normalize(searchQuery)) || 
                            normalize(p.description || '').includes(normalize(searchQuery));
      const matchesSite = !selectedSite || (p.sites && p.sites.some((ps: any) => 
        normalize(ps.site?.name) === normSelectedSite || 
        normalize(ps.site?.geographicZone?.name) === normSelectedSite
      ));
                  ...
                  >
                    ...
                    <div className="p-6 flex flex-col flex-1">
                      ...
                      <div className="bg-[#f8f9fa] border border-gray-100 rounded-2xl p-4 mb-4 mt-auto">
                        <div className="text-xl font-extrabold text-gray-900 text-center flex flex-col gap-1">
                          <div>
                            {product.price.toLocaleString('fr-FR')} CFA <span className="text-sm font-medium text-gray-400">/ {product.typeId === 1 ? 'Régime' : (product.typeId === 2 ? 'Litre' : 'Unité')}</span>
                          </div>
                          {product.typeId === 1 && product.priceTonne && parseFloat(product.priceTonne) > 0 && (
                            <div className="text-xs font-semibold text-emerald-700">
                              ({Number(product.priceTonne).toLocaleString('fr-FR')} CFA / Tonne)
                            </div>
                          )}
                        </div>
                      </div>
