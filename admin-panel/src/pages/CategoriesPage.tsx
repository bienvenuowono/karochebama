...
const CategoriesPage = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api/v1'}/catalog/categories`, config);
  ...
  const handleCreateCategory = async (e: React.FormEvent) => {
    ...
    try {
      const token = authService.getToken();
      await axios.post(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api/v1'}/catalog/categories`, { name: newCatName }, {
  ...
  const handleDeleteCategory = async (category: any) => {
    ...
    try {
      const token = authService.getToken();
      await axios.delete(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api/v1'}/catalog/categories/${category.id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchData();
    } catch (error: any) {
  ...
  const handleDeleteVariety = async (id: number) => {
    ...
    try {
      const token = authService.getToken();
      await axios.delete(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api/v1'}/catalog/categories/varieties/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchData();
  ...
  const handleCreateVariety = async (e: React.FormEvent) => {
    ...
    try {
      const token = authService.getToken();
      await axios.post(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api/v1'}/catalog/categories/varieties`, { 
        name: newVarName, 
