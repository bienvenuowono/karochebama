...
const Dashboard = () => {
  ...
  const fetchClients = async () => {
    if (stats?.clients?.list) {
      setClientsList(stats.clients.list);
    } else {
      setClientsList([]);
    }
  };
  ...
  const fetchStats = async () => {
    try {
      ...
      const res = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api/v1'}/production/analytics/dashboard-stats`, {
  ...
  return (
    <div className="space-y-8 pb-10">
      ...
      <Modal 
        ...
        title={
          reportType === 'orders' ? 'Détails des Commandes' : 
          reportType === 'categories' ? 'Détails des Catégories' : 
          reportType === 'sites' ? 'Détails des Sites Actifs' : 
          reportType === 'clients' ? 'Liste des Clients' : 
          'Messages Reçus'
        }
      >
        {(() => {
          if (!stats && reportType !== 'clients') return <div className="p-4 text-center text-slate-400">Aucune donnée disponible.</div>;

          switch (reportType) {
            case 'orders': {
              const list = stats?.orders.list || [];
              return (
                <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2">
                  {list.map((order: any) => (
                    <div key={order.id} className="p-4 bg-slate-50 rounded-2xl border border-slate-100 space-y-2">
                      <div className="flex justify-between items-start">
                        <div>
                          <p className="text-sm font-bold text-slate-900">{order.customerName || `${order.user?.firstName} ${order.user?.lastName}`}</p>
                          <p className="text-xs text-slate-400 font-medium">{new Date(order.createdAt).toLocaleString()}</p>
                        </div>
                        <span className={`px-2 py-0.5 rounded text-[10px] font-black uppercase ${order.status === 'PAID' ? 'bg-emerald-100 text-emerald-600' : 'bg-orange-100 text-orange-600'}`}>
                          {order.status}
                        </span>
                      </div>
                      <div className="text-xs text-slate-500 space-y-1">
                        <p><strong>Email :</strong> {order.customerEmail || order.user?.email || 'N/A'}</p>
                        <p><strong>Téléphone / WhatsApp :</strong> {order.customerPhone || 'N/A'} / {order.customerWhatsapp || 'N/A'}</p>
                        <p><strong>Pays :</strong> {order.customerCountry || 'N/A'}</p>
                        <p><strong>Adresse :</strong> {order.shippingAddress || 'N/A'}</p>
                        {order.notes && <p><strong>Notes :</strong> {order.notes}</p>}
                      </div>
                      <div className="border-t border-slate-200/50 pt-2 mt-2">
                        <p className="text-xs font-bold text-slate-900 mb-1">Articles :</p>
                        <div className="space-y-1">
                          {order.items?.map((item: any) => (
                            <div key={item.id} className="flex justify-between text-xs text-slate-600">
                              <span>{item.product?.name || 'Produit'} x {item.quantity}</span>
                              <span className="font-bold">{Number(item.price * item.quantity).toLocaleString()} FCFA</span>
                            </div>
                          ))}
                        </div>
                      </div>
                      <div className="border-t border-slate-200/50 pt-2 mt-2 flex justify-between items-center">
                        ...
                        <span className="text-sm font-black text-primary-600">{Number(order.totalAmount).toLocaleString()} FCFA</span>
                      </div>
                    </div>
                  ))}
                  {list.length === 0 && <div className="text-center py-6 text-slate-400 italic">Aucune commande enregistrée.</div>}
                </div>
              );
            }
            case 'categories': {
              const list = stats?.categories.health || [];
              return (
                <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2">
                  {list.map((cat: any) => (
                    <div key={cat.id} className="p-4 bg-slate-50 rounded-2xl border border-slate-100 space-y-3">
                      <p className="text-sm font-bold text-slate-900">{cat.name}</p>
                      <div className="grid grid-cols-3 gap-2 text-center">
                        <div className="bg-blue-50 p-2 rounded-xl">
                          <p className="text-xs font-medium text-blue-600">En Production</p>
                          <p className="text-lg font-black text-blue-700">{cat.stats?.en_production || 0}</p>
                        </div>
                        <div className="bg-emerald-50 p-2 rounded-xl">
                          <p className="text-xs font-medium text-emerald-600">Disponibles</p>
                          <p className="text-lg font-black text-emerald-700">{cat.stats?.disponible || 0}</p>
                        </div>
                        <div className="bg-red-50 p-2 rounded-xl">
                          <p className="text-xs font-medium text-red-600">Épuisés</p>
                          <p className="text-lg font-black text-red-700">{cat.stats?.epuise || 0}</p>
                        </div>
                      </div>
                    </div>
                  ))}
                  {list.length === 0 && <div className="text-center py-6 text-slate-400 italic">Aucune catégorie enregistrée.</div>}
                </div>
              );
            }
            case 'sites': {
              const list = stats?.sites.list || [];
              return (
                <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2">
                  {list.map((site: any) => (
                    <div key={site.id} className="p-4 bg-slate-50 rounded-2xl border border-slate-100 flex justify-between items-center">
                      <div>
                        <p className="text-sm font-bold text-slate-900">{site.name}</p>
                        <p className="text-xs text-slate-400 font-medium">Zone : {site.geographicZone?.name || 'N/A'}</p>
                      </div>
                      <span className="px-3 py-1 bg-emerald-100 text-emerald-700 text-xs font-bold rounded-xl">
                        {site.products?.length || 0} Produits
                      </span>
                    </div>
                  ))}
                  {list.length === 0 && <div className="text-center py-6 text-slate-400 italic">Aucun site actif enregistré.</div>}
                </div>
              );
            }
            case 'clients': {
              if (loadingClients) return <div className="p-4 text-center text-slate-400 italic">Chargement des clients...</div>;
              return (
                <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2">
                  {clientsList.map((client: any) => (
                    <div key={client.id} className="p-4 bg-slate-50 rounded-2xl border border-slate-100 space-y-2">
                      ...
                      <div className="text-xs text-slate-500 space-y-1">
                        ...
                        <p><strong>Pays :</strong> {client.country || 'N/A'}</p>
                        <p><strong>Adresse :</strong> {client.address || 'N/A'}</p>
                      </div>
                    </div>
                  ))}
                  {clientsList.length === 0 && <div className="text-center py-6 text-slate-400 italic">Aucun client enregistré.</div>}
                </div>
              );
            }
            case 'messages': {
              const list = stats?.messages.list || [];
              return (
                <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2">
                  {list.map((msg: any) => (
                    <div key={msg.id} className="p-4 bg-slate-50 rounded-2xl border border-slate-100 space-y-2">
                      <div className="flex justify-between items-start">
                        <div>
                          <p className="text-sm font-bold text-slate-900">{msg.name}</p>
                          <p className="text-[10px] text-slate-400 font-medium">{new Date(msg.createdAt).toLocaleString()}</p>
                        </div>
                        {msg.isRead ? (
                          <span className="px-2 py-0.5 bg-slate-100 text-slate-500 rounded text-[8px] font-black uppercase">Lu</span>
                        ) : (
                          <span className="px-2 py-0.5 bg-red-100 text-red-600 rounded text-[8px] font-black uppercase">Nouveau</span>
                        )}
                      </div>
                      <div className="text-xs text-slate-500 space-y-1">
                        <p><strong>Email :</strong> {msg.email}</p>
                        <p><strong>Téléphone :</strong> {msg.phone || 'N/A'}</p>
                        {msg.subject && <p><strong>Sujet :</strong> {msg.subject}</p>}
                      </div>
                      <p className="text-xs text-slate-700 bg-white p-3 rounded-xl border border-slate-100 mt-2 font-medium leading-relaxed">{msg.message}</p>
                    </div>
                  ))}
                  {list.length === 0 && <div className="text-center py-6 text-slate-400 italic">Aucun message de contact.</div>}
                </div>
              );
            }
            default:
