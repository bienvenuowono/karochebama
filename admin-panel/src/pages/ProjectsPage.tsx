...
const ProjectsPage = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await axios.get(`${import.meta.env.VITE_API_URL || 'https://karochebama.com/api/v1'}/projects`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        await axios.put(`${import.meta.env.VITE_API_URL || 'https://karochebama.com/api/v1'}/projects/${editingId}`, payload, {
          headers: { Authorization: `Bearer ${token}` }
        });
        alert("Projet mis à jour !");
      } else {
        await axios.post(`${import.meta.env.VITE_API_URL || 'https://karochebama.com/api/v1'}/projects`, payload, {
          headers: { Authorization: `Bearer ${token}` }
        });
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const token = authService.getToken();
      await axios.delete(`${import.meta.env.VITE_API_URL || 'https://karochebama.com/api/v1'}/projects/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchData();
    } catch (error: any) {
