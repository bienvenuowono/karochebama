...
const OrdersPage = () => {
  ...
  // Nouveaux states pour le rabais
  ...
  const fetchData = async () => {
    try {
      ...
      const responses = await Promise.allSettled([
        axios.get(`${import.meta.env.VITE_API_URL || 'https://karochebama.com/api/v1'}/catalog/orders`, config),
        axios.get(`${import.meta.env.VITE_API_URL || 'https://karochebama.com/api/v1'}/catalog/products`, config),
        axios.get(`${import.meta.env.VITE_API_URL || 'https://karochebama.com/api/v1'}/users`, config)
      ]);
  ...
  const handleUpdateStatus = async (id: number, updates: any) => {
    try {
      const token = authService.getToken();
      await axios.put(`${import.meta.env.VITE_API_URL || 'https://karochebama.com/api/v1'}/catalog/orders/${id}/status`, updates, {
  ...
  const handleUpdateDiscount = async () => {
    if (!selectedOrderForDiscount || !discountAmount) return;
    try {
      const token = authService.getToken();
      await axios.put(`${import.meta.env.VITE_API_URL || 'https://karochebama.com/api/v1'}/catalog/orders/${selectedOrderForDiscount.id}/discount`, {
    ...
    { 
      ...
      format: (val: number, row: any) => (
        <div className="flex flex-col items-end">
          <span className="font-black text-slate-900">{Number((val || 0) - (row.discount || 0)).toLocaleString()} <span className="text-[10px]">FCFA</span></span>
          {Number(row.discount || 0) > 0 && (
            <span className="text-[9px] font-bold text-orange-500 bg-orange-50 px-1.5 py-0.5 rounded-md mt-1">
              Rabais: -{Number(row.discount).toLocaleString()} FCFA
            </span>
          )}
        </div>
      )
      ...
      format: (_: any, row: any) => (
        <div className="flex items-center gap-2">
          ...
          {row?.status !== 'PAID' && (
            <button 
              onClick={() => { 
                setSelectedOrderForDiscount(row); 
                setDiscountAmount(row.discount ? row.discount.toString() : ''); 
                setIsDiscountModalOpen(true); 
              }}
              className="p-2 bg-slate-100 text-slate-400 hover:bg-orange-50 hover:text-orange-500 rounded-xl"
  ...
  return (
    <div className="space-y-8 pb-10">
      ...
      <Modal isOpen={isDiscountModalOpen} onClose={() => setIsDiscountModalOpen(false)} title="Appliquer un rabais (FCFA)">
        <div className="space-y-6">
          <p className="text-sm text-slate-500">
            Saisissez le montant en FCFA à déduire de la facture actuelle du client. Ce montant ne sera pas visible par le client sur le frontend, uniquement sur la facture générée.
          </p>
          <div>
            <label className="block text-xs font-bold text-slate-700 uppercase mb-2">Montant du rabais (FCFA)</label>
            <input 
              type="number" 
          ...
          <button 
            ...
            className="w-full py-4 bg-primary-600 hover:bg-primary-700 text-white font-black rounded-xl transition-all shadow-lg shadow-primary-500/30"
          >
            Valider le rabais
          </button>
        </div>
