...
const MediaPage = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await axios.get(`${import.meta.env.VITE_API_URL || 'https://karochebama.com/api/v1'}/media`);
      setMedia(res.data);
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        await axios.put(`${import.meta.env.VITE_API_URL || 'https://karochebama.com/api/v1'}/media/${editingId}`, payload, {
          headers: { Authorization: `Bearer ${token}` }
        });
        alert("Média mis à jour !");
      } else {
        await axios.post(`${import.meta.env.VITE_API_URL || 'https://karochebama.com/api/v1'}/media`, payload, {
          headers: { Authorization: `Bearer ${token}` }
        });
        alert("Média ajouté !");
      }
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const token = authService.getToken();
      await axios.delete(`${import.meta.env.VITE_API_URL || 'https://karochebama.com/api/v1'}/media/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchData();
    } catch (error: any) {
    ...
    { 
      ...
      format: (val: string, row: any) => (
        <div className="flex items-center gap-3">
          {row.type === 'IMAGE' ? (
             <img src={val ? (val.startsWith('http') ? val : `${import.meta.env.VITE_API_URL || 'https://karochebama.com/api/v1'}${val}`) : ''} alt="media" className="w-12 h-12 rounded-lg object-cover bg-slate-100" />
          ) : (
