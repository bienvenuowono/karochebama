...
const ProfilePage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.put(`${import.meta.env.VITE_API_URL || 'https://karochebama.com/api/v1'}/users/${user.id}`, updateData, config);
  ...
  const getPhotoUrl = () => {
    ...
    if (formData.photoUrl) {
      return formData.photoUrl.startsWith('http') ? formData.photoUrl : `${import.meta.env.VITE_API_URL || 'https://karochebama.com/api/v1'}${formData.photoUrl}`;
    }
    return null;
  };
