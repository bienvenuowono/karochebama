...
import { PlusCircle, Sprout, Trash2, Edit } from 'lucide-react';
...
const ActivitiesPage = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await axios.get(`${import.meta.env.VITE_API_URL || 'https://karochebama.com/api/v1'}/activities`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        await axios.put(`${import.meta.env.VITE_API_URL || 'https://karochebama.com/api/v1'}/activities/${editingId}`, payload, {
          headers: { Authorization: `Bearer ${token}` }
        });
        alert("Activité mise à jour !");
      } else {
        await axios.post(`${import.meta.env.VITE_API_URL || 'https://karochebama.com/api/v1'}/activities`, payload, {
          headers: { Authorization: `Bearer ${token}` }
        });
      ...
      setFormData({ title: '', description: '', imageUrl: '', status: 'PUBLISHED' });
      setImageFile(null);
      setEditingId(null);
      fetchData();
    } catch (error: any) {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const token = authService.getToken();
      await axios.delete(`${import.meta.env.VITE_API_URL || 'https://karochebama.com/api/v1'}/activities/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchData();
    } catch (error: any) {
    ...
    { 
      ...
      format: (val: string, row: any) => (
        <div className="flex items-center gap-3">
          {row.imageUrl ? (
            <img src={row.imageUrl.startsWith('http') ? row.imageUrl : `${import.meta.env.VITE_API_URL || 'https://karochebama.com/api/v1'}${row.imageUrl}`} alt={val} className="w-10 h-10 rounded-xl object-cover" />
          ) : (
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-900 font-outfit tracking-tight">Activités</h1>
          <p className="text-sm text-slate-500 mt-1">Gérez les activités affichées sur le site vitrine.</p>
        </div>
        <button onClick={() => setIsDialogOpen(true)} className="px-4 py-2 bg-primary-500 text-white rounded-xl text-sm font-bold hover:bg-primary-600 flex items-center gap-2 shadow-lg shadow-primary-500/20">
          <PlusCircle size={18} /> Nouvelle Activité
        </button>
      </div>

      <DataTable columns={columns} data={activities} loading={loading} />

      <Modal isOpen={isDialogOpen} onClose={closeDialog} title={editingId ? "Modifier l'Activité" : "Nouvelle Activité"}>
