...
const ActivitiesPage = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api/v1'}/activities`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        await axios.put(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api/v1'}/activities/${editingId}`, payload, {
          headers: { Authorization: `Bearer ${token}` }
        });
        alert("Activité mise à jour !");
      } else {
        await axios.post(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api/v1'}/activities`, payload, {
          headers: { Authorization: `Bearer ${token}` }
        });
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const token = authService.getToken();
      await axios.delete(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api/v1'}/activities/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchData();
    } catch (error: any) {
