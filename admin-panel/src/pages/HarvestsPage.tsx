...
const HarvestsPage = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const [fRes, hRes, pRes] = await Promise.all([
        axios.get(`${import.meta.env.VITE_API_URL || 'https://karochebama.com/api/v1'}/production/harvests/forecasts`, config),
        axios.get(`${import.meta.env.VITE_API_URL || 'https://karochebama.com/api/v1'}/production/harvests/history`, config),
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      const token = authService.getToken();
      await axios.post(`${import.meta.env.VITE_API_URL || 'https://karochebama.com/api/v1'}/production/harvests`, 
        formData, 
