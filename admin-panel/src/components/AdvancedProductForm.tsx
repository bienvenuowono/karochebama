...
const AdvancedProductForm = ({ onSubmit, onCancel, initialData }: any) => {
  ...
  const fetchMetadata = async () => {
    try {
      ...
      const [catRes, siteRes] = await Promise.all([
        axios.get(`${import.meta.env.VITE_API_URL || 'https://karochebama.com/api/v1'}/catalog/categories`, config),
        axios.get(`${import.meta.env.VITE_API_URL || 'https://karochebama.com/api/v1'}/production/sites`, config)
      ]);
  ...
  useEffect(() => {
    if (initialData && categories.length > 0) {
      setFormData({
        ...
        priceTonne: initialData.priceTonne?.toString() || '',
        description: initialData.description || '',
      });

      if (initialData.imageUrl) {
        setMainPreview(initialData.imageUrl.startsWith('http') ? initialData.imageUrl : `${import.meta.env.VITE_API_URL || 'https://karochebama.com/api/v1'}${initialData.imageUrl}`);
      }

      if (initialData.gallery && Array.isArray(initialData.gallery)) {
        setGalleryPreviews(initialData.gallery.map((url: string) => url.startsWith('http') ? url : `${import.meta.env.VITE_API_URL || 'https://karochebama.com/api/v1'}${url}`));
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const priceTonneNum = parseFloat(formData.priceTonne) || 0;
    payloadData.priceKg = (priceTonneNum / 1000).toString();

    Object.keys(payloadData).forEach(key => {
      if (key === 'siteIds') data.append(key, JSON.stringify(payloadData[key]));
      else data.append(key, payloadData[key]);
    });
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8 pb-20 w-full">
      ...
      <div className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-sm space-y-6">
        ...
        <div className="space-y-6">
          <div><label className={labelClass}>Prix de vente Public REGIME (CFA)</label><input required type="number" className={inputClass} placeholder="ex: 5000" value={formData.price} onChange={(e) => setFormData({...formData, price: e.target.value})} /></div>
          {isAgri && <div><label className={labelClass}>Prix au Tonne (FCFA)</label><input type="number" className={inputClass} placeholder="ex: 500000" value={formData.priceTonne} onChange={(e) => setFormData({...formData, priceTonne: e.target.value})} /></div>}
        </div>
      </div>
